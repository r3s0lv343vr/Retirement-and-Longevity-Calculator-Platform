...
const HOUSING_KEYS: (keyof CalculatorInput)[] = [
  "ccrcRentAnnual",
  "ccrcStartAge",
...
export function HousingForm({ values, onChange, onSubmit, loading, error }: Props) {
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <section className="card">
        ...
        <p className="mt-1 text-sm leading-relaxed text-muted">
          Ages, savings, lifestyle, healthcare, and guaranteed income. Lifestyle should already include the housing you
          have now. Pension at $0 is skipped.
        </p>
        <div className="mt-5 grid items-end gap-x-6 gap-y-6 sm:grid-cols-2">
          {MAIN_KEYS.map((key) => (
            <HousingField key={key} fieldKey={key} values={values} onChange={setField} wide={key === "pensionCola"} />
          ))}
        </div>
      </section>
      ...
      <section className="card">
        <h2 className="font-serif text-xl leading-tight text-pine">Facility rents to compare</h2>
        <p className="mt-1 text-sm leading-relaxed text-muted">
          Each rent is its own path. Stay home uses none of them. They are not stacked on one run, and they are not
          added as more cells on How long.
        </p>
        <div className="mt-5 grid items-end gap-x-6 gap-y-6 sm:grid-cols-2">
          {HOUSING_KEYS.map((key) => (
            <HousingField key={key} fieldKey={key} values={values} onChange={setField} />
          ))}
        </div>
      </section>
      ...
      >
        {loading ? "Comparing housing paths…" : "Compare housing paths"}
      </button>
    </form>
  );
}
