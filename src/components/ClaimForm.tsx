...
export function ClaimForm({ values, onChange, onSubmit, loading, error }: Props) {
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <section className="card">
        ...
        <p className="mt-1 text-sm leading-relaxed text-muted">
          Enter the Social Security check at the start age you know. This run scales that check to 67 and 70 and
          compares how far the same spending path lasts. Pension at $0 is skipped.
        </p>
        <div className="mt-5 grid items-end gap-x-6 gap-y-6 sm:grid-cols-2">
          {MAIN_KEYS.map((key) => (
            <ClaimField key={key} fieldKey={key} values={values} onChange={setField} wide={key === "pensionCola"} />
          ))}
        </div>
      </section>
      ...
      <details className="card group">
        ...
        <div className="mt-4 grid gap-x-5 gap-y-5 sm:grid-cols-2">
          {ADVANCED_KEYS.map((key) => (
            <ClaimField key={key} fieldKey={key} values={values} onChange={setField} />
          ))}
        </div>
      </details>
      ...
      >
        {loading ? "Comparing claim ages…" : "Compare 67 and 70"}
      </button>
    </form>
  );
}
