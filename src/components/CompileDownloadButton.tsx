...
export function CompileDownloadButton({ result }: Props) {
  ...
  async function compileAndDownload() {
    ...
    setError(null);
    try {
      const { downloadOutlookPdf } = await import("@/lib/report/compileOutlookPdf");
      downloadOutlookPdf(result);
    } catch {
  ...
  return (
    <section className="card">
      ...
      <p className="mt-2 max-w-3xl text-sm leading-relaxed text-muted">
        Builds a PDF of the completed report in your browser — status, comfortable living, the entered-plan figures,
        capital months, year-by-year rows, and the inputs you used. Nothing is stored on a server.
      </p>
      <button
        type="button"
