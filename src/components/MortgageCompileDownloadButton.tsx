...
export function MortgageCompileDownloadButton({ result }: Props) {
  ...
  async function compileAndDownload() {
    ...
    setError(null);
    try {
      const { downloadMortgagePdf } = await import("@/lib/report/compileMortgagePdf");
      downloadMortgagePdf(result);
    } catch {
  ...
  return (
    <section className="card">
      ...
      <p className="mt-2 max-w-3xl text-sm leading-relaxed text-muted">
        Builds a PDF in your browser — the buffer at years 1, 5, and 10, the compare if you ran one, the yearly
        amortization, and the inputs. Nothing is stored on a server.
      </p>
      <button
        type="button"
